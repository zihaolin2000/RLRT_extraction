...
use RMF, only: getRMF_parSet, waleckaShift, m_nucleon
...
write(f,'(A,i2,A)') 'RMF_set',getRMF_parSet(),'.dat'

open(1,file=trim(f),status='unknown')
write(1,*)'# rhobar, fm^-3:    p_fermi, GeV:   rhos, fm^-3:   m^*, GeV:'
write(1,*)'# (cont.)  E/A-m_N, GeV:  p, GeV/fm^3:  S, GeV:  V, GeV:   U, GeV:'
...
!   rhobar = i*0.0001
   ...
   shift = waleckaShift (rhobar, em0=mass_seed, rhoscalar=rhos,     &

  ...
  subroutine threeBody(partPert, partReal, time)
       ...
       do j=lbound(partPert,dim=2),ubound(partPert,dim=2)
          ...
          if (fullensemble) then
             if (nukSearch_VE.and.localEnsemble) then
                call VolumeElements_NukSearch(partPert(i,j), &
                     GetRadiusNukSearch(), &
          ...
          flagOk = finalCheck((/partPert(i,j),&
               scatterPartner1, scatterPartner2/), finalState, .false.)
          if (.not. flagOk) cycle
    ...
    do i=lbound(partReal,dim=1), ubound(partReal,dim=1)
       do j=lbound(partReal,dim=2), ubound(partReal,dim=2)
          ...
          if (fullensemble) then
             if (nukSearch_VE.and.localEnsemble) then
                call VolumeElements_NukSearch(partReal(i,j), &
                     GetRadiusNukSearch(),&
                     proton1,proton2,neutron1,neutron2, flagOk)
             else
                call NukSearch(partReal(i,j), partReal, &
                     proton1, proton2, neutron1, neutron2, flagOk)
             end if
          else
             call NukSearch(partReal(i,j), partReal(i:i,:), &
                  proton1, proton2, neutron1, neutron2, flagOk)
          end if
...
!!$          end if
          ...
          call make_3Body_Collision (partReal(i,j), proton1, proton2, neutron1, &
            & neutron2, scatterPartner1, scatterPartner2, finalstate, flagOk)
          if (.not. flagOk) cycle
          ...
          flagOk = finalCheck((/partReal(i,j),&
               scatterPartner1, scatterPartner2/), finalState, .false.)
          if (.not. flagOk) cycle

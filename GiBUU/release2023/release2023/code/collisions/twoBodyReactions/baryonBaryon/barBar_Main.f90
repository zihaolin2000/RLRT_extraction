...
contains
  ...
  ! PURPOSE
  ! This routine is the main routine for baryon-baryon scattering and its
  ! cross sections.
  ! Determines total and elastic cross section and makes a Monte-Carlo
  ! decision for a specific reaction channnel. This leads to a definition of
  ! ID and charge of teilchenOut, which is the final state vector.
  ! INPUTS
  ...
  ! * character(len=*), optional      :: plotFlag  -- Switch on plotting the
  !   Xsections (indicates the prefix of th eoutput files)
  ! OUTPUT
  ! * type(particle), dimension(1:3) :: partOut        -- outgoing particles
  ! * real                           :: sigmaTot       -- total Xsection
  ! * real                           :: sigmaElast     -- elastic Xsection
  ! * logical                        :: pauliIncluded  -- true = cross section
  ...
  subroutine XsectionBarBar(srts, partIn, mediumAtColl, partOut, sigmaTot, sigmaElast, pauliIncluded, plotFlag)
    ...
    character(len=*), intent(in), optional     :: plotFlag
    ...
    real, dimension(nucleon:F37_1950) :: sigmaNukBar      ! -> nucleon+baryon.
    ...
    real, dimension(Delta:F37_1950)   :: sigmaDeltaBar    ! -> Delta+baryon,
    ! summed over all possible charge states of the deltas.
    real, dimension(1:18)             :: Sigma_BYK        ! -> Bar Hyperon Kaon
    real, dimension(1:4)              :: Sigma_NNPion     ! "N N Pion" production as a background term
    real, dimension(1:3)              :: Sigma_NNV        ! vector meson production ("N N omega/phi") as a background term
    real                              :: sigma_np_deta    ! n+p -> d+eta
    real, dimension(1:4)              :: Sigma_YN         ! Hyperon N -> Hyperon N
    ...
    if ( (hadron(partIn(1)%ID)%charm.ne.0) &
         .or.(hadron(partIn(2)%ID)%charm.ne.0) ) then
       ! At least one charmed baryon as scattering partner!
       ! Here are no cross sections implemented for this incoming particles
       return

    else if ( (hadron(partIn(1)%ID)%strangeness.ne.0)&
         .and.(hadron(partIn(2)%ID)%strangeness.ne.0) ) then
       ! Two strange baryons as scattering partner!
       ! Here are no cross sections implemented for this incoming particles
       return

    else if ( (hadron(partIn(1)%ID)%strangeness.ne.0)&
         .or.(hadron(partIn(2)%ID)%strangeness.ne.0) ) then
       ...
       if (partIn(1)%Id==nucleon .or. partIn(2)%Id==nucleon) then
          ...
          if (partIn(1)%Id==nucleon .and. partIn(2)%Id==nucleon) then
             ...
             ! Evaluate background cross sections for NN -> NN omega/phi
             ! (currently no res. with omega decay channel are produced in
             ! NN coll.)
          ...
          ! No cross sections implemented for these incoming particles!
          sigmaElast=0.
          sigmaTot=0.
          return
       end if
       ...
       sigmaTot = Sum(sigmaNukBar) + sum(sigmaDeltaBar) + SUM(Sigma_NNPion) &
            + sigma_np_deta + sum(Sigma_NNV) + Sum(Sigma_BYK)
    ...
    case (0) !strange=0: baryon+baryon -> baryon+baryon
       ...
       ! BB -> B Hyperon Kaon
       !             with B=N,Delta; Hyperon=Lambda,Sigma; Kaon=K^{+,0}
    ...
    write(*,*) 'Problem in XsectionBarBar.'
    write(*,*) 'We did not find a channel in the Monte-Carlo-Decision.'
    write(*,*) 'x= ', x, ' wahrscheinlichkeit=', wahrscheinlichkeit
    stop
  ...
  contains
    ...
    subroutine PlotSigmas
      ...
      write(222,'(68ES13.4E3)') srts, plab, sigmaTot, &
           sum(sigmaNukBar(3:18)), sum(sigmaNukBar(19:31)), &
           sigmaNukBar(nucleon:F37_1950), &
           sum(sigmaDeltaBar(3:18)), sum(sigmaDeltaBar(19:31)), &
           sigmaDeltaBar(Delta:F37_1950)
      write(223,'(26ES13.4E3)') srts, plab, sigmaTot, Sigma_NNPion, &
           Sigma_NNV, sum(Sigma_BYK), Sigma_BYK
  ...
  ! PURPOSE
  ! This routine returns the cross section for "n p -> d eta" in mb, as a
  ! function of sqrt(s).
  ! We use a tabulated spline fit of the data points, which is being read
  ! from an input file ("eta_deuteron_spline.txt").
  ...
  real function eta_deuteron(srts)
    ...
    real, save :: sigma_field(100,1:2), Qmax, one_dQ
    real :: Q, w
    integer :: iQ
    ...
    if (Q < 0.) then
       ...
       iQ = floor(Q*one_dQ)+1
       if (iQ<100) then
          w = (Q-sigma_field(iQ,1))*one_dQ
          eta_deuteron = (1.-w)*sigma_field(iQ,2)+w*sigma_field(iQ+1,2)
       else
          eta_deuteron = sigma_field(100,2)
       end if
    end if

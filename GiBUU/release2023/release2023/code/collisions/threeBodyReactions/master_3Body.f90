...
contains
  ...
  subroutine make_3Body_Collision(SingleParticle,proton1,proton2,neutron1,neutron2,scatterPartner1,&
    ...
    if (debug) then
       write(*,*)
       write(*,'(A,3F8.3,2I4)') 'Regarded Particle:',Singleparticle%pos, SingleParticle%ID, SingleParticle%charge
       if (associated(proton1)) write(*,'(A,3F8.3,2I4)') 'Proton #1:', proton1%pos,proton1%ID,proton1%charge
       if (associated(proton2))write(*,'(A,3F8.3,2I4)') 'Proton #2:',proton2%pos,proton1%ID,proton2%charge
       if (associated(neutron1))write(*,'(A,3F8.3,2I4)') 'Neutron #1:',neutron1%pos,neutron1%ID,neutron1%charge
       if (associated(neutron2))write(*,'(A,3F8.3,2I4)') 'Neutron #2:',neutron2%pos,neutron2%ID,neutron2%charge
    end if
...
!    call setToDefault(finalState)
    ...
    if (SingleParticle%ID.eq.pion.and.positionNNPI) then
       finalState%pos(1)=singleParticle%pos(1)
       finalState%pos(2)=singleParticle%pos(2)
       finalState%pos(3)=singleParticle%pos(3)
    end if
  ...
  subroutine pionAbsorption(singleParticle,proton1,proton2,neutron1,neutron2,scatterPartner1,scatterPartner2, &
    ...
    real :: Epion, rhoProton, rhoNeutron, srts, F, x, wahrscheinlichkeit, gamma(0:2), eNucleon(1:2)
    ...
    density=densityAt(SingleParticle%pos)
    ...
    if (associated(neutron1).and.(Associated(neutron2))) then
       ...
       srts=sqrt((Sum(enucleon)+Epion)**2 &
            &   -Dot_Product(SingleParticle%mom(1:3)+neutron1%mom(1:3)+neutron2%mom(1:3), &
            &                SingleParticle%mom(1:3)+neutron1%mom(1:3)+neutron2%mom(1:3)))
       ...
       F=RatioNNPion_NN(neutron1,neutron2,SingleParticle)  ! In-medium correction factor
       gamma(0)=gamma(0)*F
    end if
    ! Set NP channel
    if (associated(neutron1).and.(Associated(proton1))) then
       ...
       srts=sqrt((Sum(enucleon)+Epion)**2 &
            &   -Dot_Product(SingleParticle%mom(1:3)+neutron1%mom(1:3)+proton1%mom(1:3), &
            &                SingleParticle%mom(1:3)+neutron1%mom(1:3)+proton1%mom(1:3)))
       if (srts.gt.NNPION_NN_maxSrts) then
         if (Debug) write(*,*) 'srts=',srts,'>',NNPION_NN_maxSrts,'=NNPION_NN_maxSrts. No absorption'
         return
       end if
       ...
       if (debug) write(*,*) 'Energies:',Epion,Enucleon, 'Srts=',srts
       F=RatioNNPion_NN(neutron1,proton1,SingleParticle)  ! In-medium correction factor
       gamma(1)=gamma(1)*F
    end if
    ! Set PP channel
    if (associated(proton1).and.(Associated(proton2))) then
       ...
       srts=sqrt((Sum(enucleon)+Epion)**2 &
            &   -Dot_Product(SingleParticle%mom(1:3)+proton1%mom(1:3)+proton2%mom(1:3), &
            &                SingleParticle%mom(1:3)+proton1%mom(1:3)+proton2%mom(1:3)))
       if (srts.gt.NNPION_NN_maxSrts) then
         if (Debug) write(*,*) 'srts=',srts,'>',NNPION_NN_maxSrts,'=NNPION_NN_maxSrts. No absorption'
         return
       end if
       ...
       if (debug) write(*,*) 'Energies:',Epion,Enucleon, 'Srts=',srts
       F=RatioNNPion_NN(proton1,proton2,SingleParticle)  ! In-medium correction factor
       gamma(2)=gamma(2)*F
    end if
...
!    endif
  ...
  subroutine deltaAbsorption(singleParticle,proton1,proton2,neutron1,neutron2,scatterPartner1,scatterPartner2, &
    ...
    if (get_MediumSwitch_Delta()) then
       ...
       momentum=singleParticle%mom(1:3)
...
!    successFlag=.true.
    ...
    case default
       write(*,*) 'Strange charge in delta absorption', singleParticle%charge,singleParticle%anti,&
            &singleParticle%ID,singleParticle%mom
    end select
  ...
  subroutine setFinalState(particle1,particle2,particle3,preFinalState,finalState,successFlag)
    ...
    momentum_calc(0:3)=particle1%mom(0:3)+particle2%mom(0:3)+particle3%mom(0:3)
    ...
    position=(particle1%pos+particle2%pos+particle3%pos)/3.
    ...
    ! (4a) Intialize output
    call setToDefault(finalState)
    if (.not.particle1%pert.and.(.not.particle2%pert).and.(.not.particle3%pert)) then
  ...
  subroutine setKinematics(srts,srts_vacuum,momentum_calc,betaToLRF,betaToCM,medium_AtCollision, &
    ...
    case (1)
       ...
       finalState(1)%mom=momentum_calc
    case (2)

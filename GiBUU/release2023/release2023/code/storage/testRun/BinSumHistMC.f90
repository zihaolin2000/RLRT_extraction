...
program BinSumHistMC
  ...
  do
     ...
     if (nDirs==0) then
        call fetchHistMC(hSum,file=trim(dir)//trim(arg), &
             add=addFak,mul=mulFak, flagOK=flagOK)
        write(*,*) 'add,mul = ',addFak,mulFak
     else
        call fetchHistMC(hTmp,file=trim(dir)//trim(arg), flagOK=flagOK)
     end if
     if (.not.flagOK) cycle

     if (nDirs>0) then
        call sumHistMC(hSum, hTmp)
     end if
  ...
  call writeHistMC(hSum, file=trim(arg0), add=addFak,mul=mulFak/nDirs)

end program BinSumHistMC

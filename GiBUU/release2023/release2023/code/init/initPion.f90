...
contains
  ...
  contains
    ...
    subroutine setKinematics
      ...
      Teilchen(j,index)%anti=.false.
      Teilchen(j,index)%pert=.true.
      Teilchen(j,index)%prodTime=0.
      Teilchen(j,index)%mass=mPi
      Teilchen(j,index)%mom(0)=ekin_lab+Teilchen(j,index)%mass
      ! pion is initialized moving in positive z-direction:
      Teilchen(j,index)%mom(1:3)=(/0.,0.,Sqrt(Teilchen(j,index)%mom(0)**2-Teilchen(j,index)%mass**2)/)
      ! assume vacuum dispersion relation:
      Teilchen(j,index)%vel(1:3)=teilchen(j,index)%mom(1:3)/teilchen(j,index)%mom(0)
    ...
    subroutine setPosition(impact)
      ...
      if (impact_parameter.ge.0.) then
         ...
         teilchen(j,index)%pos=(/impact,0.,-distance/)
         ...
         if (randomNumber.le.pInnerDisk) then
            impact=rn()
            teilchen(j,index)%pos(1)=sqrt(impact)*bmax_InnerDisk*cos(phi)
            teilchen(j,index)%pos(2)=sqrt(impact)*bmax_InnerDisk*sin(phi)
            teilchen(j,index)%pos(3)=-distance
            ...
            impact=rn()
            teilchen(j,index)%pos(1)=sqrt(impact*(bmax_OuterRing**2-bmax_InnerDisk**2)&
                 &                                      +bmax_InnerDisk**2)*cos(phi)
            teilchen(j,index)%pos(2)=sqrt(impact*(bmax_OuterRing**2-bmax_InnerDisk**2)&
    ...
    subroutine CoulombCorrect
      ...
      if (debug) then
         ...
         write(*,*) 'position=', teilchen(j,index)%pos
         write(*,*) '4-momentum=', teilchen(j,index)%mom
      end if

      teilchen(j,index)%pos(3)=-coulombdistance

      call Coulpropa(teilchen(j,index)%pos(1:3), teilchen(j,index)%mom(1:3), &
           teilchen(j,index)%charge, teilchen(j,index)%mass, &
           targetNuc%charge, distance)

      !Assume vacuum dispersion relation:
      teilchen(j,index)%vel(1:3)=teilchen(j,index)%mom(1:3)/FreeEnergy(teilchen(j,index))
    ...
    subroutine momentumCorrect(successFlag)
      ...
      cpot=0.
      if (UseCoulomb) then
         cpot = emfoca(teilchen(j,index)%pos,teilchen(j,index)%mom(1:3),teilchen(j,index)%charge,teilchen(j,index)%ID)

         if (kineticEnergy(teilchen(j,index))-cPot.lt.0) then
            write(*,*) "Error in Initpion"
            write(*,*) "Energy too small:",teilchen(j,index)%mom
            write(*,*) "Cannot initiliaze pions at",teilchen(j,index)%pos
            stop
         end if

         ! Correct momentum  p**2+m**2=(E-V_coulomb)**2 :
         teilchen(j,index)%mom(1:3)=(/0.,0.,SQRT((ekin_lab+teilchen(j,index)%mass&
              &                         -cPot)**2-teilchen(j,index)%mass**2)/)
         teilchen(j,index)%mom(0)=ekin_lab+teilchen(j,index)%mass-cpot
      ...
      if (debug) then
         ...
         write(*,'(A,4F9.6)') "In medium momentum:",teilchen(j,index)%mom
         write(*,*)
      end if
    ...
    subroutine RechneImpuls(partIn,energy,success)
      ...
      do j=0,maxSteps
         ...
         do i=-1,1
            teilchen%mom(1:2)=0.
            teilchen%mom(3)=partIn%mom(3)+i*dp
            call energyDetermination(teilchen)
            f(i)=teilchen%mom(0)-energy
         end do

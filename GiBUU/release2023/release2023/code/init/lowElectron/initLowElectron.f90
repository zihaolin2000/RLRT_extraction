...
module initLowElectron
  ...
  ! * 2: we use W_min and W_max as input. For this we assume the
  ...
  !****************************************************************************
  !****g* initLowElectron/theta_lf
  ! SOURCE
  real,save :: theta_lf=10.
  !
  ! PURPOSE
  ! Theta scattering angle of outgoing electron with respect to the incoming
  ! one.
  !
  ...
  !****************************************************************************
  !****g* initLowElectron/delta_energy_lf
  ! SOURCE
  ...
  !****************************************************************************
  !****g* initLowElectron/energy_lf_min
  ! SOURCE
  ...
  !****************************************************************************
  !****g* initLowElectron/energy_lf_max
  ! SOURCE
  ...
  !****************************************************************************
  !****g* initLowElectron/Do_1Pi
  ! SOURCE
  logical,save :: Do_1Pi=.true.
  !
  ! PURPOSE
  ! Switch for including or excluding direct Single pion production processes.
  ! If resonances are included (Do_Res=.true.), then only the background part
  ! is included.
  !****************************************************************************

  !****************************************************************************
  !****g* initLowElectron/Do_2Pi
  ! SOURCE
  logical,save :: Do_2Pi=.true.
  !
  ! PURPOSE
  ! Switch for including or excluding direct Double pion production processes.
  ! If resonances are included (Do_Res=.true.), then only the background part
  ! is included.
  !****************************************************************************

  !****************************************************************************
  !****g* initLowElectron/Do_DIS
  ! SOURCE
  ...
  !****************************************************************************
  !****g* initLowElectron/Do_2p2hDelta
  ! SOURCE
  ...
  !****************************************************************************
  !****g* initLowElectron/nuclearTarget_corr
  ! SOURCE
  ...
  !****************************************************************************
  !****g* initLowElectron/minEnergy_1pi
  ! SOURCE
  ...
  !****************************************************************************
  !****g* initLowElectron/low_Ele_incXsection
  ! SOURCE
...
contains
  ...
  !****************************************************************************
  !****s* initLowElectron/init_lowElectron
  ! NAME
  ...
  subroutine init_lowElectron(realParticles,pertParticles,raiseEnergy,targetNuc)
    ...
    type(particle), intent(inout), dimension(:,:) :: pertParticles

    type(tNucleus), pointer                       :: targetNuc
    logical, intent(in)                           :: raiseEnergy
    ...
    if (targetNuc%mass.gt.1) then
       if (absVec(targetNuc%vel).lt.1E-4) then
          ...
          write(*,*) 'Error: Target nucleus is in motion in init_lowElectron. velocity=',targetNuc%vel
    ...
    case default
       write(*,*) 'Wrong runType in initLowElectron.f90', runType
       write(*,*) 'STOP!'
       stop
    end select

    ! Write kinematics to file
    open(40,file='initLowElectron_protocol.log',position='Append')
    ...
    ensembleLoop: do j=lbound(realParticles,dim=2),ubound(realParticles,dim=2)
       indexLoop: do i=lbound(realParticles,dim=1),ubound(realParticles,dim=1)
          ...
          if (eN%boson%mom(0).lt.minEnergy_1pi) then
             Do_1Pi_switch=.false.
          else
          ...
          if (nuclearTarget) then
             ...
             finalState%perweight=finalState%perweight*nuclearFluxFactor_correction(realParticles(i,j)%mom,electron_in)
             xsection= xsection*nuclearFluxFactor_correction(realParticles(i,j)%mom,electron_in)
          end if
...
!!$          end if
  ...
  !****************************************************************************
  !****f* initLowElectron/le_get_energy_lf
  ! NAME
  ...
  !****************************************************************************
  !****f* initLowElectron/le_get_energy_li
  ! NAME
  ...
  !****************************************************************************
  !****f* initLowElectron/le_get_theta_lf
  ! NAME
  ...
  subroutine initInput
    ...
    select case (inputType)
       ...
       case default
          write(*,*) 'Wrong inputType in initLowElectron.f90', inputType
          write(*,*) 'STOP!'
          stop
    end select
  ...
  !****************************************************************************
  !****s* initLowElectron/resetFirstEvent
  ! NAME
  ...
  !****************************************************************************
  !****if* initLowElectron/getFirstEvent
  ! NAME
  ! integer function getFirstEvent()
  ! PURPOSE
  ! Each event gets another entry in %firstEvent. This function is used to
  ! set firstEvent.
  !****************************************************************************
  integer function getFirstEvent()
    first=first+1
  ...
  !****************************************************************************
  !****f* initLowElectron/le_get_FirstEventRange
  ! NAME
  ! function le_get_FirstEventRange()
  ! PURPOSE
  ! Each event gets another entry in %firstEvent. This entry is an integer
  ! value between 1 and number of events.
  ! This function returns the upper and lower limits of the range of
  ! %firstEvent.
  ! OUTPUT
  ...
  !****************************************************************************
  !****f* initLowElectron/le_get_QSquared
  ! NAME
  ...
  !****************************************************************************
  !****s* initLowElectron/writeOriginXS
  ! NAME

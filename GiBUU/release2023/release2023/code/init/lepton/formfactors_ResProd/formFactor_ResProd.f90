...
contains
 ...
 !   use readresparam, only: MADelta
    ...
    case (1)
       write(*,'(a,3F12.4)') ' using Adlers form for delta C_5^A with fit parameters a, b, C5A0corr: ', &
                             &  aDelta, bDelta, C5A0corr
    case (2)
  ...
  function getFormfactor_Res (Qs, bare_mass, resID, targetCharge, process, FF_set)  &
    ...
    if (FF_ResProd.eq.0) then
       ...
       if (abs(process).ne.EM) call axialformfactor(formfactorfield(5:8),Qs,resID,targetCharge, &
       & process,FF_set)

    else if (FF_ResProd.eq.1) then
       call vectorformfactor_Lalakulich(formfactorfield(1:4),Qs,resID,targetCharge,process, &
       & FF_set)
       if (abs(process).ne.EM) call axialformfactor_Lalakulich(formfactorfield(5:8),Qs,resID,  &

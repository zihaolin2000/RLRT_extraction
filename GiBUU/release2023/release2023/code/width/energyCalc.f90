...
contains
  ...
  ! NAME
  ! subroutine energyDetermination(part,betaToCF,countMax,error_out,
  ...
  !    defined.
  ! * integer, OPTIONAL :: countMax  -- maximal number of cycles to find
  ...
  !****************************************************************************
  subroutine energyDetermination(part, betaToCF, countMax_in, error_out, check, warn, ForbidCoulomb, skipTestE0, flagOK, verbose)
    ...
    logical, optional, intent(in) :: check, warn, ForbidCoulomb, skipTestE0, verbose
    logical, optional, intent(out):: flagOK
    ...
    logical :: checkE,success,outOfBounds_offshell,warn_,DoCoulomb,skipE0,verb
    type(dichte) :: density
...
!!$    end if
    ...
    if (present(flagOK)) flagOK = success

    if (verb .and. checkE .and. abs(part%mom(0)-E0)>delta_E) then
  ...
  contains
    ...
    logical function vary_mass_offshell()
      ...
      do
         ...
         if (part2%mom(0).gt.10.) then
            ! FAILURE
            if (verb) write(*,*) 'FAILURE in vary_mass_offshell'
    ...
    logical function bisection(E1,E2)
      ...
      if (f(1)*f(2)>0) then
         ...
         bisection=.false.
         ! call plot_f(part)
         ! call traceback('Stop in EnergyDetermination (Bisection)', -1)
         return
      end if
    ...
    subroutine errorMessage
      ...
      write(*,'(a,1P,3E20.7)') 'SQRT( f(1:3) )       :', SQRT(abs(f))
      write(*,'(a,1P,3E20.7)') 'f(1:3)               :', f
      write(*,'(a,1P,3E20.7)') 'E(1:3)               :', E
      write(*,'(a,i4)')        'Id of particle       :', part%Id
      write(*,'(a,i4)')        'Charge of particle   :', part%charge
      write(*,'(a,1P,E12.4)')  'Mass of particle     :', part%mass
      write(*,'(a,1P,4E12.4)') 'Momentum of particle :', part%mom
      write(*,'(a,1P,E12.4)')  'Abs. mom. of particle:', absMom(part)
      write(*,'(a,L2)')        'Perturbative         :', part%pert
      write(*,'(a,1P,E12.4)')  'Offshell parameter   :', part%offshellPar
      if (present(betaToCF)) then
         write(*,'(a,1P,3E12.4)')'beta to Calculation frame :', betaToCF
      else
